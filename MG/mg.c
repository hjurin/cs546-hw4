                  ...
                  double c[4], int k)
{
  ...
  #pragma omp parallel for schedule(guided) default(shared) private(i1,i2,i3,r1,r2)
  for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
      for (i1 = 0; i1 < n1; i1++) {
                  ...
                  double a[4], int k)
{
  ...
  #pragma omp parallel for schedule(guided) default(shared) private(i1,i2,i3,u1,u2)
  for (i3 = 1; i3 < n3-1; i3++) {
    for (i2 = 1; i2 < n2-1; i2++) {
      for (i1 = 0; i1 < n1; i1++) {
                   ...
                   void *ou, int n1, int n2, int n3, int k)
{
  ...
  if (n1 != 3 && n2 != 3 && n3 != 3) {
    #pragma omp parallel for schedule(guided) default(shared) private(i1,i2,i3,z1,z2,z3)
...
static void zran3(void *oz, int n1, int n2, int n3, int nx1, int ny1, int k)
{
  ...
  // fill array
  //---------------------------------------------------------------------
  #pragma omp parallel for schedule(guided) default(shared) private(i2,i3,x1,xx,rdummy) \
                           shared(e2,e3,d1,a1)
  for (i3 = 1; i3 < e3; i3++) {
    x1 = starts[i3];
    for (i2 = 1; i2 < e2; i2++) {
  ...
  */
  ...
  #pragma omp parallel for schedule(guided) default(shared) private(i1,i2,i3)
  for (i3 = 0; i3 < n3; i3++) {
    for (i2 = 0; i2 < n2; i2++) {
      for (i1 = 0; i1 < n1; i1++) {
        z[i3][i2][i1] = 0.0;
